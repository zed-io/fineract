...
exports.LoanChargeService = void 0;
...
class LoanChargeService {
     ...
     */
    async addCharge(loanId, chargeData, userId) {
        logger_1.logger.info('Adding charge to loan', {
            loanId,
            chargeData,
            userId
        });
        return db_1.db.transaction(async (client) => {
            try {
                ...
                const loanResult = await client.query('SELECT loan_status, currency_code, principal_amount, principal_outstanding_derived FROM loan WHERE id = $1', [loanId]);
                ...
                let chargeDefinition = {};
                if (chargeData.chargeId) {
                    const chargeDefResult = await client.query('SELECT name, currency_code, charge_calculation_enum, charge_time_enum, charge_payment_mode_enum, amount, is_penalty, is_active FROM m_charge WHERE id = $1', [chargeData.chargeId]);
                ...
                if (calculationType !== loanAdvanced_1.LoanChargeCalculationType.FLAT) {
                    const percentage = chargeData.percentage || 0;
                    amountPercentageAppliedTo = this.getAmountPercentageAppliedTo(calculationType, loanPrincipal, outstandingPrincipal);
                ...
                const insertResult = await client.query(`INSERT INTO m_loan_charge (
          ...
          RETURNING id`, [
                    ...
                    this.mapChargeTimeTypeToEnum(chargeData.chargeTimeType || loanAdvanced_1.LoanChargeTimeType.SPECIFIED_DUE_DATE),
                ...
                if (chargeData.chargeTimeType === loanAdvanced_1.LoanChargeTimeType.DISBURSEMENT &&
                ...
                logger_1.logger.error('Error adding charge to loan', { loanId, error });
     ...
     */
    async applyCharge(client, loanId, chargeId, userId) {
        try {
            // Get charge details
            const chargeResult = await client.query(`SELECT id, amount, is_penalty, is_paid, is_waived, amount_outstanding, charge_time_enum
         FROM m_loan_charge WHERE id = $1 AND loan_id = $2 AND is_active = true`, [chargeId, loanId]);
            ...
            // Get loan details
            const loanResult = await client.query(`SELECT id, loan_status, currency_code, fee_charges_outstanding_derived, 
        ...
        RETURNING id`, [
            ...
            await client.query(`INSERT INTO m_loan_charge_payment (
          loan_charge_id, loan_transaction_id, amount
        ) VALUES ($1, $2, $3)`, [chargeId, transactionId, charge.amount_outstanding]);
         ...
         WHERE id = $3`, [feeChargesOutstanding, penaltyChargesOutstanding, loanId]);
            // Mark charge as applied in relevant ways based on type
            if (charge.charge_time_enum === this.mapChargeTimeTypeToEnum(loanAdvanced_1.LoanChargeTimeType.DISBURSEMENT)) {
                // For disbursement charges, mark as paid
                await client.query(`UPDATE m_loan_charge SET
            is_paid = true,
            amount_paid = amount,
           ...
           WHERE id = $1`, [chargeId]);
            ...
            logger_1.logger.error('Error applying charge', { loanId, chargeId, error });
     ...
     * @returns True if successful
     */
    async waiveCharge(loanId, chargeId, userId) {
        logger_1.logger.info('Waiving loan charge', {
            loanId,
            chargeId,
            userId
        });
        return db_1.db.transaction(async (client) => {
            try {
                // Get charge details
                const chargeResult = await client.query(`SELECT id, amount, is_penalty, is_paid, is_waived, amount_outstanding
           FROM m_loan_charge WHERE id = $1 AND loan_id = $2 AND is_active = true`, [chargeId, loanId]);
                if (chargeResult.rowCount === 0) {
                    throw new Error('Charge not found or not active');
                }
          ...
          RETURNING id`, [
           ...
           WHERE id = $1`, [chargeId]);
                ...
                logger_1.logger.error('Error waiving charge', { loanId, chargeId, error });
     ...
     */
    calculateChargeAmount(charge, loanPrincipal, loanInterest, outstandingPrincipal) {
        try {
            if (charge.chargeCalculationType === loanAdvanced_1.LoanChargeCalculationType.FLAT) {
            ...
            switch (charge.chargeCalculationType) {
                case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_AMOUNT:
                    baseAmount = loanPrincipal;
                    break;
                case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_AMOUNT_AND_INTEREST:
                    baseAmount = loanPrincipal + loanInterest;
                    break;
                case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_INTEREST:
                    baseAmount = loanInterest;
                    break;
                case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_DISBURSEMENT_AMOUNT:
                    baseAmount = loanPrincipal;
                    break;
                case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_TOTAL_OUTSTANDING:
            ...
            logger_1.logger.error('Error calculating charge amount', { charge, error });
     ...
     */
    applyChargesToSchedule(loanId, schedule, charges) {
        logger_1.logger.info('Applying charges to schedule', {
            loanId,
        ...
        try {
            ...
            for (const charge of charges) {
                ...
                switch (charge.chargeTimeType) {
                    case loanAdvanced_1.LoanChargeTimeType.DISBURSEMENT:
                        this.applyDisbursementCharge(newSchedule, charge);
                        break;
                    case loanAdvanced_1.LoanChargeTimeType.SPECIFIED_DUE_DATE:
                        this.applySpecifiedDueDateCharge(newSchedule, charge);
                        break;
                    case loanAdvanced_1.LoanChargeTimeType.INSTALLMENT_FEE:
                        this.applyInstallmentFeeCharge(newSchedule, charge);
                        break;
                    case loanAdvanced_1.LoanChargeTimeType.OVERDUE_INSTALLMENT:
                        this.applyOverdueInstallmentCharge(newSchedule, charge);
                        break;
                    case loanAdvanced_1.LoanChargeTimeType.OVERDUE_MATURITY:
            ...
            logger_1.logger.error('Error applying charges to schedule', { loanId, error });
     ...
     */
    applyDisbursementCharge(schedule, charge) {
     ...
     */
    applySpecifiedDueDateCharge(schedule, charge) {
        if (!charge.dueDate) {
            return;
        }
     ...
     */
    applyInstallmentFeeCharge(schedule, charge) {
     ...
     */
    applyOverdueInstallmentCharge(schedule, charge) {
     ...
     */
    applyOverdueMaturityCharge(schedule, charge) {
     ...
     */
    mapChargeCalculationTypeToEnum(calculationType) {
        switch (calculationType) {
            case loanAdvanced_1.LoanChargeCalculationType.FLAT:
                return 1;
            case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_AMOUNT:
                return 2;
            case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_AMOUNT_AND_INTEREST:
     ...
     */
    mapChargeCalculationEnum(enumValue) {
        switch (enumValue) {
            case 1:
                return loanAdvanced_1.LoanChargeCalculationType.FLAT;
            case 2:
                return loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_AMOUNT;
            case 3:
                return loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_AMOUNT_AND_INTEREST;
            case 4:
                return loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_INTEREST;
            case 5:
                return loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_DISBURSEMENT_AMOUNT;
            case 6:
                return loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_TOTAL_OUTSTANDING;
            default:
                return loanAdvanced_1.LoanChargeCalculationType.FLAT;
        }
    }
     ...
     * @returns The corresponding database enum value
     */
    mapChargeTimeTypeToEnum(timeType) {
        switch (timeType) {
            case loanAdvanced_1.LoanChargeTimeType.DISBURSEMENT:
                return 1;
            case loanAdvanced_1.LoanChargeTimeType.SPECIFIED_DUE_DATE:
            ...
            case loanAdvanced_1.LoanChargeTimeType.OVERDUE_MATURITY:
                return 5;
            case loanAdvanced_1.LoanChargeTimeType.OVERDUE_ON_LOAN_MATURITY:
                return 6;
            case loanAdvanced_1.LoanChargeTimeType.TRANCHE_DISBURSEMENT:
                return 7;
            default:
                return 2;
        }
     ...
     */
    mapChargeTimeEnum(enumValue) {
        switch (enumValue) {
            case 1:
                return loanAdvanced_1.LoanChargeTimeType.DISBURSEMENT;
            case 2:
                return loanAdvanced_1.LoanChargeTimeType.SPECIFIED_DUE_DATE;
            case 3:
                return loanAdvanced_1.LoanChargeTimeType.INSTALLMENT_FEE;
            case 4:
                return loanAdvanced_1.LoanChargeTimeType.OVERDUE_INSTALLMENT;
            case 5:
                return loanAdvanced_1.LoanChargeTimeType.OVERDUE_MATURITY;
            case 6:
                return loanAdvanced_1.LoanChargeTimeType.OVERDUE_ON_LOAN_MATURITY;
            case 7:
                return loanAdvanced_1.LoanChargeTimeType.TRANCHE_DISBURSEMENT;
            default:
                return loanAdvanced_1.LoanChargeTimeType.SPECIFIED_DUE_DATE;
        }
    }
     ...
     */
    getAmountPercentageAppliedTo(calculationType, loanPrincipal, outstandingPrincipal) {
        switch (calculationType) {
            case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_AMOUNT:
            case loanAdvanced_1.LoanChargeCalculationType.PERCENT_OF_DISBURSEMENT_AMOUNT:
...
exports.LoanChargeService = LoanChargeService;
