...
exports.LoanDelinquencyService = void 0;
...
class LoanDelinquencyService {
     ...
     */
    async processDelinquency(loanId, force = false) {
        logger_1.logger.info('Processing loan delinquency', { loanId, force });
        try {
            // Get loan details
            const loanResult = await db_1.db.query(`SELECT l.id, l.loan_status, l.expected_maturity_date, 
         ...
         WHERE l.id = $1`, [loanId]);
            ...
            const overdueResult = await db_1.db.query(`SELECT MIN(ls.due_date) as oldest_overdue_date,
         ...
         AND ls.completed_derived = false`, [loanId]);
            ...
            const delinquencyResult = await db_1.db.query(`SELECT id, classification, delinquent_days, delinquent_amount,
         delinquent_date, last_delinquent_date, oldest_unpaid_installment_date,
         is_active
         FROM m_loan_delinquency
         WHERE loan_id = $1 AND is_active = true`, [loanId]);
            let delinquencyId = null;
            let previousClassification = loanAdvanced_1.DelinquencyClassification.NO_DELINQUENCY;
            ...
            if (delinquencyId) {
                ...
                await db_1.db.query(`UPDATE m_loan_delinquency SET
            classification = $1,
            delinquent_days = $2,
            delinquent_amount = $3,
           ...
           WHERE id = $5`, [
                ...
                // Create new record
                const insertResult = await db_1.db.query(`INSERT INTO m_loan_delinquency (
          ...
          RETURNING id`, [
            ...
            logger_1.logger.error('Error processing delinquency', { loanId, error });
     ...
     */
    async clearDelinquency(loanId) {
        try {
            const result = await db_1.db.query(`UPDATE m_loan_delinquency SET
          is_active = false,
          resolved_date = NOW()
         WHERE loan_id = $1 AND is_active = true`, [loanId]);
            ...
            logger_1.logger.error('Error clearing delinquency', { loanId, error });
     ...
     */
    async processAllDelinquencies() {
        logger_1.logger.info('Processing delinquency for all active loans');
        try {
            // Get all active loans with outstanding balance
            const loansResult = await db_1.db.query(`SELECT id
         FROM loan
         ...
         AND total_outstanding_derived > 0`);
            ...
            for (const loan of loans) {
                try {
                    ...
                    logger_1.logger.error('Error processing delinquency for loan', { loanId: loan.id, error });
                    // Continue with next loan
                }
            }
            logger_1.logger.info(`Delinquency processing completed`, {
            ...
            logger_1.logger.error('Error processing all delinquencies', { error });
     ...
     */
    async getDelinquentLoans(classification, activeOnly = true) {
        try {
            let query = `
      ...
      `;
            ...
            logger_1.logger.error('Error getting delinquent loans', { classification, error });
     ...
     */
    async getDelinquencyHistory(loanId) {
        try {
            const result = await db_1.db.query(`SELECT id, loan_id, classification, delinquent_days, delinquent_amount,
         delinquent_date, last_delinquent_date, oldest_unpaid_installment_date,
         is_active, previous_classification, classification_changed_date,
         resolved_date, created_date
         FROM m_loan_delinquency
         WHERE loan_id = $1
         ORDER BY delinquent_date DESC`, [loanId]);
            return result.rows;
        }
        catch (error) {
            logger_1.logger.error('Error getting delinquency history', { loanId, error });
     ...
     */
    getDelinquencyClassification(daysOverdue) {
        if (daysOverdue < 30) {
            return loanAdvanced_1.DelinquencyClassification.NO_DELINQUENCY;
        }
        else if (daysOverdue < 60) {
            return loanAdvanced_1.DelinquencyClassification.DELINQUENT_30;
        }
        else if (daysOverdue < 90) {
            return loanAdvanced_1.DelinquencyClassification.DELINQUENT_60;
        }
        else if (daysOverdue < 120) {
            return loanAdvanced_1.DelinquencyClassification.DELINQUENT_90;
        }
        else if (daysOverdue < 150) {
            return loanAdvanced_1.DelinquencyClassification.DELINQUENT_120;
        }
        else if (daysOverdue < 180) {
            return loanAdvanced_1.DelinquencyClassification.DELINQUENT_150;
        }
        else {
            return loanAdvanced_1.DelinquencyClassification.DELINQUENT_180;
        }
    }
    /**
     ...
     */
    async applyDelinquencyPenalty(loanId, classification) {
        try {
            // Check if delinquency charge is configured for this classification
            const chargeResult = await db_1.db.query(`SELECT c.id, c.name, c.amount, c.currency_code, 
         ...
         WHERE dc.classification = $1 AND c.active = true`, [classification]);
            ...
            // Get loan currency
            const loanResult = await db_1.db.query('SELECT currency_code FROM loan WHERE id = $1', [loanId]);
            if (loanResult.rowCount === 0) {
                return false;
            }
            ...
            if (charge.currency_code !== loanCurrency) {
                logger_1.logger.warn('Delinquency charge currency does not match loan currency', {
                    loanId,
                    loanCurrency,
            ...
            // Insert loan charge
            await db_1.db.query(`INSERT INTO m_loan_charge (
          ...
          created_by, created_date, amount_outstanding
            ...
            logger_1.logger.error('Error applying delinquency penalty', { loanId, classification, error });
            // Don't throw - non-critical operation
            return false;
        }
    }
}
exports.LoanDelinquencyService = LoanDelinquencyService;
