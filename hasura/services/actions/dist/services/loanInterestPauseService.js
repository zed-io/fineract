...
exports.LoanInterestPauseService = void 0;
 ...
 */
class LoanInterestPauseService {
    /**
     ...
     */
    async createInterestPause(loanId, startDate, endDate, reasonId, reasonComment, userId) {
        logger_1.logger.info('Creating loan interest pause', {
            loanId,
            startDate,
            endDate,
            userId
        ...
        return db_1.db.transaction(async (client) => {
            try {
                // Get loan details
                const loanResult = await client.query('SELECT loan_status, disbursed_on_date FROM loan WHERE id = $1', [loanId]);
                ...
                const overlapResult = await client.query(`SELECT id FROM m_loan_interest_pause
           WHERE loan_id = $1 
           AND is_active = true
           AND (
             ...
             (start_date >= $2 AND end_date <= $3)
                ...
                const insertResult = await client.query(`INSERT INTO m_loan_interest_pause (
          ...
          RETURNING id`, [
              ...
              updated_by = $1,
              updated_on = NOW()
             WHERE id = $2`, [userId, loanId]);
                }
                return pauseId;
            }
            catch (error) {
                logger_1.logger.error('Error creating interest pause', { loanId, error });
     ...
     */
    async cancelInterestPause(pauseId, userId, cancellationReason) {
        logger_1.logger.info('Cancelling loan interest pause', {
            pauseId,
            userId
        });
        return db_1.db.transaction(async (client) => {
            try {
                // Get pause details
                const pauseResult = await client.query(`SELECT p.id, p.loan_id, p.start_date, p.end_date, p.is_active, 
           p.is_period_active
           FROM m_loan_interest_pause p
           WHERE p.id = $1`, [pauseId]);
                ...
                await client.query(`UPDATE m_loan_interest_pause SET
            is_active = false,
            ...
            updated_by = $2,
            updated_date = NOW()
           WHERE id = $3`, [cancellationReason, userId, pauseId]);
                // If pause was active, check if there are other active pauses
                if (pause.is_period_active) {
                    const activeResult = await client.query(`SELECT COUNT(*) as active_count
             FROM m_loan_interest_pause
             WHERE loan_id = $1
             AND is_active = true
             AND is_period_active = true`, [pause.loan_id]);
                ...
                updated_by = $1,
                updated_on = NOW()
               WHERE id = $2`, [userId, pause.loan_id]);
                    }
                }
                return true;
            }
            catch (error) {
                logger_1.logger.error('Error cancelling interest pause', { pauseId, error });
     ...
     */
    async getInterestPausePeriods(loanId, activeOnly = false) {
        try {
            let query = `
      ...
      `;
            ...
            logger_1.logger.error('Error getting interest pause periods', { loanId, error });
     ...
     */
    async hasActiveInterestPause(loanId, date) {
        try {
            const checkDate = date ? new Date(date) : new Date();
            const formattedDate = checkDate.toISOString().split('T')[0];
            const result = await db_1.db.query(`SELECT p.id, p.loan_id, p.start_date, p.end_date, p.reason_id,
         r.code as reason_code, r.name as reason_name, p.reason_comment,
         p.is_active, p.is_period_active, p.created_by_user_id,
         ...
         LIMIT 1`, [loanId, formattedDate]);
            ...
            logger_1.logger.error('Error checking for active interest pause', { loanId, date, error });
     ...
     */
    async updateAllInterestPauseStatuses() {
        logger_1.logger.info('Updating interest pause statuses for all loans');
        return db_1.db.transaction(async (client) => {
            try {
                ...
                const toActivateResult = await client.query(`SELECT DISTINCT l.id
           FROM loan l
           ...
           AND p.end_date >= $1`, [today]);
               ...
               AND is_active = true
               AND start_date <= $2
               AND end_date >= $2`, [loanId, today]);
                    }
                    activated = loanIds.length;
                }
                // Find loans that should now have inactive pause
                const toDeactivateResult = await client.query(`SELECT l.id
           FROM loan l
           WHERE l.interest_pause_status = 'active'
           AND l.loan_status = 'active'
           AND NOT EXISTS (
             ...
             AND p.end_date >= $1
               ...
               WHERE id = $1`, [loanId]);
                ...
                updated_date = NOW()
               WHERE loan_id = $1
               AND is_period_active = true`, [loanId]);
                    }
                    deactivated = loanIds.length;
                }
                logger_1.logger.info('Interest pause status update completed', {
                    activated,
                    deactivated
                });
                ...
                logger_1.logger.error('Error updating interest pause statuses', { error });
     ...
     */
    async calculateTotalInterestFreeDays(loanId) {
        try {
            // Get all active pause periods
            const result = await db_1.db.query(`SELECT start_date, end_date
         FROM m_loan_interest_pause
         WHERE loan_id = $1
         AND is_active = true
         ORDER BY start_date`, [loanId]);
            if (result.rowCount === 0) {
                return 0;
            }
            ...
            for (const range of mergedRanges) {
                totalDays += (0, loan_1.calculateDaysBetween)(range.start, range.end) + 1; // +1 to include end date
            }
            return totalDays;
        }
        catch (error) {
            logger_1.logger.error('Error calculating total interest-free days', { loanId, error });
     ...
     */
    async calculateInterestAdjustment(loanId, interestRate, outstandingPrincipal) {
        try {
            ...
            logger_1.logger.error('Error calculating interest adjustment', {
                loanId,
                interestRate,
...
exports.LoanInterestPauseService = LoanInterestPauseService;
