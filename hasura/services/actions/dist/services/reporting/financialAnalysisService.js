...
exports.generateIncomeStatementReport = generateIncomeStatementReport;
exports.generateBalanceSheetReport = generateBalanceSheetReport;
exports.generateFinancialRatiosReport = generateFinancialRatiosReport;
exports.generateInterestIncomeReport = generateInterestIncomeReport;
exports.generateFeeIncomeReport = generateFeeIncomeReport;
 ...
 */
async function generateIncomeStatementReport(fromDate, toDate, officeId, currencyCode, includeComparative = false) {
    logger_1.logger.info('Generating income statement report', {
    ...
    try {
        ...
        const incomeQuery = await db_1.db.query(`WITH income_accounts AS (
        SELECT 
          ga.id,
          ga.name,
          ga.gl_code,
      ...
      ORDER BY gl_code`, params);
        ...
        const expenseQuery = await db_1.db.query(`WITH expense_accounts AS (
        SELECT 
          ga.id,
          ga.name,
          ga.gl_code,
 ...
 */
async function generateBalanceSheetReport(asOfDate, officeId, currencyCode, includeComparative = false) {
    logger_1.logger.info('Generating balance sheet report', {
    ...
    try {
        ...
        const assetQuery = await db_1.db.query(`WITH asset_accounts AS (
        SELECT 
          ga.id,
          ga.name,
          ga.gl_code,
      ...
      ORDER BY gl_code`, params);
        ...
        const liabilityQuery = await db_1.db.query(`WITH liability_accounts AS (
        SELECT 
          ga.id,
          ga.name,
          ga.gl_code,
      ...
      ORDER BY gl_code`, params);
        ...
        const equityQuery = await db_1.db.query(`WITH equity_accounts AS (
        SELECT 
          ga.id,
          ga.name,
          ga.gl_code,
      ...
      ORDER BY gl_code`, params);
        ...
        const liabilitiesByCategory = new Map();
        ...
        const equityByCategory = new Map();
        let totalEquity = 0;
 ...
 */
async function generateFinancialRatiosReport(asOfDate, officeId) {
    logger_1.logger.info('Generating financial ratios report', { asOfDate, officeId });
    try {
        ...
        const incomeStatement = await generateIncomeStatementReport(yearStartDate, asOfDate, officeId);
      ...
      WHERE 
        ...
        ${officeId ? 'AND l.office_id = $3' : ''}`, officeId ? [asOfDate, yearStartDate, officeId] : [asOfDate, yearStartDate]);
        // Get loan loss reserve and liquidity data
        const accountBalancesQuery = await db_1.db.query(`WITH account_balances AS (
        SELECT 
          ga.id,
          ga.name,
          ga.gl_code,
      ...
      FROM 
        account_balances`, officeId ? [asOfDate, officeId] : [asOfDate]);
        // Get expense breakdown
        const expenseBreakdownQuery = await db_1.db.query(`WITH expense_breakdown AS (
        SELECT 
          ga.tag_id,
          COALESCE(SUM(CASE 
            WHEN je.entry_date BETWEEN $1 AND $2 
      ...
      FROM 
        expense_breakdown`, officeId ? [yearStartDate, asOfDate, officeId] : [yearStartDate, asOfDate]);
        // Get client and staff counts for productivity metrics
        const countQuery = await db_1.db.query(`SELECT 
 ...
 */
async function generateInterestIncomeReport(fromDate, toDate, officeId, currencyCode) {
    logger_1.logger.info('Generating interest income report', { fromDate, toDate, officeId, currencyCode });
    try {
        // Build params array for query
        const params = [fromDate, toDate];
        let paramIndex = 3;
        ...
        const interestQuery = await db_1.db.query(`SELECT 
      ...
      ORDER BY 
        ...
        const trendQuery = await db_1.db.query(`SELECT 
 ...
 */
async function generateFeeIncomeReport(fromDate, toDate, officeId, currencyCode) {
    logger_1.logger.info('Generating fee income report', { fromDate, toDate, officeId, currencyCode });
    try {
        // Build params array for query
        const params = [fromDate, toDate];
        let paramIndex = 3;
        ...
        const feeQuery = await db_1.db.query(`SELECT 
      ...
      WHERE 
        ...
        const feeTypeQuery = await db_1.db.query(`SELECT 
