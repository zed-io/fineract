...
exports.generateAccountDetailsReport = generateAccountDetailsReport;
 ...
 */
async function generateBalanceSheet(officeId, asOnDate) {
    logger_1.logger.info('Generating balance sheet', { officeId, asOnDate });
    try {
        // Get office name
      ...
      ORDER BY 
        account_code
    `;
        const result = await db_1.db.query(query, [officeId, asOnDate]);
        ...
        return {
            type: accounting_1.FinancialStatementType.BALANCE_SHEET,
            office: officeName,
            toDate: asOnDate,
            sections,
            totalAssets,
        ...
        logger_1.logger.error('Error generating balance sheet', error);
 ...
 */
async function generateIncomeStatement(officeId, fromDate, toDate) {
    logger_1.logger.info('Generating income statement', { officeId, fromDate, toDate });
    try {
        // Get office name
        const officeQuery = await db_1.db.query(`SELECT name FROM fineract_default.office WHERE id = $1`, [officeId]);
        if (officeQuery.rows.length === 0) {
      ...
      ORDER BY 
        account_code
    `;
        const result = await db_1.db.query(query, [officeId, fromDate, toDate]);
        // Group accounts by type
        const accountsByType = {};
        for (const row of result.rows) {
            const accountType = row.account_type;
        ...
        return {
            type: accounting_1.FinancialStatementType.INCOME_STATEMENT,
            office: officeName,
            fromDate,
            toDate,
            sections,
        ...
        logger_1.logger.error('Error generating income statement', error);
 ...
 */
async function generateAccountDetailsReport(accountId, officeId, fromDate, toDate) {
    logger_1.logger.info('Generating account details report', { accountId, officeId, fromDate, toDate });
    try {
        // Get account details
        const accountQuery = await db_1.db.query(`SELECT id, name, gl_code, account_type
       ...
       WHERE id = $1`, [accountId]);
        ...
        const openingBalanceQuery = await db_1.db.query(`WITH account_transactions AS (
        SELECT
      ...
      FROM
        account_transactions`, [accountId, officeId, fromDate, account.account_type]);
        const openingBalance = parseFloat(openingBalanceQuery.rows[0].opening_balance) || 0;
        // Get transactions for the period
        const transactionsQuery = await db_1.db.query(`SELECT
        je.id,
        je.transaction_id,
        je.entry_date,
        je.type,
      ...
      WHERE
        ...
        AND je.reversed = false
      ORDER BY
        je.entry_date, je.id`, [account.account_type, accountId, officeId, fromDate, toDate]);
        ...
        logger_1.logger.error('Error generating account details report', error);
        return {
            success: false,
