...
import { 
  InterestRecalculationConfiguration, 
 ...
 */
export class LoanInterestRecalculationService {
  ...
  recalculateSchedule(
    loanId: string,
    originalSchedule: LoanSchedule,
    terms: LoanApplicationTerms,
    recalculationConfig: InterestRecalculationConfiguration,
    transactionDate: string,
    transactionAmount: number,
    isPayment: boolean = true
  ): LoanSchedule {
    logger.info('Recalculating loan schedule', { 
      loanId, 
      transactionDate, 
      transactionAmount, 
      isPayment 
    ...
    try {
      if (!recalculationConfig || recalculationConfig.interestRecalculationCompoundingMethod === InterestRecalculationCompoundingMethod.NONE) {
        logger.debug('Interest recalculation not configured, returning original schedule');
      ...
      const pastPeriods: LoanSchedulePeriod[] = [];
      let futurePeriods: LoanSchedulePeriod[] = [];
      ...
      let outstandingPrincipal = this.calculateOutstandingPrincipal(
        pastPeriods, 
        transactionAmount, 
        isPayment
      );
      ...
      futurePeriods = this.recalculateFuturePeriods(
        futurePeriods,
      ...
      logger.error('Error recalculating schedule', { loanId, error });
   ...
   */
  private calculateOutstandingPrincipal(
    pastPeriods: LoanSchedulePeriod[],
    transactionAmount: number,
    isPayment: boolean
  ): number {
   ...
   */
  private recalculateFuturePeriods(
    futurePeriods: LoanSchedulePeriod[],
    ...
    switch (rescheduleStrategyMethod) {
      case RescheduleStrategyMethod.REDUCE_NUMBER_OF_INSTALLMENTS:
        return this.recalculateWithReducedInstallments(
          futurePeriods, 
          outstandingPrincipal, 
          terms
        );
        
      case RescheduleStrategyMethod.REDUCE_EMI_AMOUNT:
        return this.recalculateWithReducedEMI(
          futurePeriods, 
          outstandingPrincipal, 
          terms
        );
        
      case RescheduleStrategyMethod.RESCHEDULE_NEXT_REPAYMENTS:
        return this.recalculateWithRescheduledRepayments(
          futurePeriods, 
          outstandingPrincipal, 
          terms,
          transactionDate
   ...
   * @returns The recalculated future periods
   */
  private recalculateWithReducedInstallments(
    futurePeriods: LoanSchedulePeriod[],
    outstandingPrincipal: number,
    terms: LoanApplicationTerms
  ): LoanSchedulePeriod[] {
    ...
    const recalculatedPeriods: LoanSchedulePeriod[] = [];
    ...
    for (let i = 0; i < periodsNeeded; i++) {
      ...
      const recalculatedPeriod: LoanSchedulePeriod = {
   ...
   * @returns The recalculated future periods
   */
  private recalculateWithReducedEMI(
    futurePeriods: LoanSchedulePeriod[],
    outstandingPrincipal: number,
    terms: LoanApplicationTerms
  ): LoanSchedulePeriod[] {
   ...
   * @returns The recalculated future periods
   */
  private recalculateWithRescheduledRepayments(
    futurePeriods: LoanSchedulePeriod[],
    outstandingPrincipal: number,
    terms: LoanApplicationTerms,
    transactionDate: string
    ...
    let nextRepaymentDate = addPeriods(
      transactionDate, 
      repaymentEvery, 
    ...
    for (let i = 0; i < futurePeriods.length; i++) {
      ...
      const periodDueDate = i === 0 
        ? nextRepaymentDate 
        : addPeriods(recalculatedPeriods[i - 1].dueDate, repaymentEvery, repaymentFrequencyType);
      ...
      if (interestMethod === 'flat') {
        ...
        const daysInPeriod = calculateDaysBetween(periodFromDate, periodDueDate);
        const daysInYear = calculateDaysInYear(
          periodDueDate.getFullYear(), 
          terms.daysInYearType || DaysInYearType.DAYS_365
        );
   ...
   */
  generateInterestRecalculationPeriods(
    loanId: string,
    terms: LoanApplicationTerms,
    recalculationConfig: InterestRecalculationConfiguration,
    ...
    logger.info('Generating interest recalculation periods', { 
      loanId, 
      disbursementDate, 
      endDate 
    });
    ...
    try {
      ...
      if (!recalculationConfig || 
          recalculationConfig.interestRecalculationCompoundingMethod === InterestRecalculationCompoundingMethod.NONE) {
        return periods;
      }
      ...
      const frequencyType = recalculationCompoundingFrequencyType === RecalculationCompoundingFrequency.SAME_AS_REPAYMENT_PERIOD
      ...
      const frequencyInterval = recalculationCompoundingFrequencyInterval || 
                               (recalculationCompoundingFrequencyType === RecalculationCompoundingFrequency.SAME_AS_REPAYMENT_PERIOD
      ...
      while (currentDate <= targetEndDate) {
        ...
        // Move to next period
        currentDate = addPeriods(currentDate, frequencyInterval, frequencyType);
      }
      
      return periods;
    } catch (error) {
      logger.error('Error generating interest recalculation periods', { loanId, error });
   ...
   */
  private mapRecalculationFrequencyToPeriodFrequencyType(
    recalculationFrequency: RecalculationCompoundingFrequency | RecalculationRestFrequency
  ): PeriodFrequencyType {
    switch (recalculationFrequency) {
      case RecalculationCompoundingFrequency.DAILY:
      case RecalculationRestFrequency.DAILY:
        return PeriodFrequencyType.DAYS;
        
      case RecalculationCompoundingFrequency.WEEKLY:
      case RecalculationRestFrequency.WEEKLY:
        return PeriodFrequencyType.WEEKS;
        
      case RecalculationCompoundingFrequency.MONTHLY:
      case RecalculationRestFrequency.MONTHLY:
