...
export class ReportService {
   ...
   */
  async getReportDefinition(reportId: string): Promise<ReportDefinition> {
    try {
      ...
      const parametersResult = await db.query(
        `SELECT p.id, p.report_id, p.name, p.display_name, p.parameter_type, 
      ...
      const columnsResult = await db.query(
        `SELECT c.id, c.report_id, c.name, c.display_name, c.data_type, 
      ...
      const parameters: ReportParameter[] = parametersResult.rows.map(row => ({
        id: row.id,
      ...
      // Map report columns
      const columns: ReportColumn[] = columnsResult.rows.map(row => ({
        id: row.id,
        reportId: row.report_id,
        name: row.name,
        displayName: row.display_name,
      ...
      logger.error('Error getting report definition', { reportId, error });
      throw new ReportError(`Failed to get report definition: ${error.message}`);
    }
  }
  ...
  async getAllReportDefinitions(
    ...
    try {
      ...
      logger.error('Error getting all report definitions', { category, activeOnly, error });
      throw new ReportError(`Failed to get report definitions: ${error.message}`);
    }
  }
  ...
  async executeReport(request: ReportExecutionRequest): Promise<ReportExecutionResult> {
    ...
    try {
      ...
      if (!report.isActive) {
        throw new ReportError(`Report '${report.name}' is not active`);
      }
      ...
      // Execute query
      logger.debug('Executing report query', { reportId: report.id, sql: query.sql });
      ...
      const paging = request.pagination ? {
        ...
        totalRecords: parseInt(result.rows[0]?.total_count?.toString() || data.length.toString()),
        totalPages: Math.ceil(
          parseInt(result.rows[0]?.total_count?.toString() || data.length.toString()) / 
      ...
      if (error instanceof ReportNotFoundError || 
          error instanceof InvalidReportParameterError ||
          error instanceof ReportError) {
        throw error;
      }
      
      throw new ReportExecutionError(`Failed to execute report: ${error.message}`);
    }
  }
   ...
   */
  async exportReport(request: ReportExecutionRequest): Promise<{
    fileName: string;
    contentType: string;
    data: Buffer;
  }> {
    try {
      ...
      switch (format) {
        ...
        default:
          throw new ReportError(`Unsupported export format: ${format}`);
      }
    } catch (error) {
      ...
      if (error instanceof ReportError ||
          ...
          error instanceof InvalidReportParameterError ||
          error instanceof ReportExecutionError) {
        throw error;
      }
      
      throw new ReportError(`Failed to export report: ${error.message}`);
    }
  }
    ...
    return db.transaction(async (client) => {
      try {
        ...
        for (const param of report.parameters) {
          await client.query(
            `INSERT INTO m_report_parameter (
        ...
        logger.error('Error creating report definition', { report, error });
        throw new ReportError(`Failed to create report definition: ${error.message}`);
      }
    });
  }
  ...
  private async logReportExecution(
    ...
    try {
      await db.query(
        `INSERT INTO m_report_execution_history (
      ...
      logger.error('Error logging report execution', { reportId, error });
    }
  }
  ...
  private validateParameters(
    ...
    for (const param of report.parameters) {
      if (param.isMandatory && 
        ...
        throw new InvalidReportParameterError(`Missing mandatory parameter: ${param.name}`);
      }
      ...
      switch (param.parameterType) {
        case ParameterType.NUMBER:
          if (typeof value !== 'number') {
            throw new InvalidReportParameterError(
        ...
        case ParameterType.DATE_RANGE:
          ...
          if (param.parameterType === ParameterType.DATE_RANGE) {
      ...
      if (param.validationRegex && typeof value === 'string') {
        ...
        if (!regex.test(value)) {
          throw new InvalidReportParameterError(
            `Parameter ${param.name} fails validation regex: ${param.validationRegex}`
          );
        }
      }
    }
  ...
  private async prepareQuery(
    ...
    throw new ReportError('Report definition does not contain SQL or query definition');
  }
  ...
  private prepareRawSqlQuery(
    ...
    for (const param of report.parameters) {
      ...
      if (value !== undefined && value !== null) {
        ...
        } else if (param.parameterType === ParameterType.MULTI_SELECT) {
    ...
    if (request.pagination) {
      const pageSize = Math.min(
        request.pagination.pageSize || this.DEFAULT_PAGE_SIZE,
        this.MAX_PAGE_SIZE
      );
  ...
  private buildQueryFromDefinition(
    ...
    if (queryDef.filters) {
      const whereClause = this.buildFilterClause(
        queryDef.filters, report, request.parameters, params, paramIndex
      );
    ...
    if (queryDef.havingFilters) {
      const havingClause = this.buildFilterClause(
        queryDef.havingFilters, report, request.parameters, params, paramIndex
      );
    ...
    // Finally use default column sorting
      ...
      const defaultSortColumns = report.columns
        ...
        .sort((a, b) => 
          (a.defaultSortOrder || 0) - (b.defaultSortOrder || 0)
        );
      
      if (defaultSortColumns.length > 0) {
        const sortClauses = defaultSortColumns.map(col => {
          const direction = col.defaultSortDirection || SortDirection.ASC;
  ...
  private buildFilterClause(
    ...
    for (const filter of filterGroup.filters) {
      ...
      if ('operator' in filter) {
        const nestedClause = this.buildFilterClause(
          filter, report, paramValues, params, nextIndex
        );
   ...
   */
  private calculateTotals(data: any[], columns: ReportColumn[]): { [key: string]: any } {
  ...
  private validateReportDefinition(report: ReportDefinition): void {
    ...
    if (!report.name) {
      throw new ReportError('Report name is required');
    }
    
    if (!report.displayName) {
      throw new ReportError('Report display name is required');
    }
    
    if (!report.category) {
      throw new ReportError('Report category is required');
    }
    
    // Check that either reportSql or reportQuery is provided
    if (!report.reportSql && !report.reportQuery) {
      throw new ReportError('Either report SQL or query definition is required');
    }
    
    // Validate query definition if provided
    if (report.reportQuery) {
      if (!report.reportQuery.mainTable) {
        throw new ReportError('Query definition must specify a main table');
      }
      
      if (!report.reportQuery.columns || report.reportQuery.columns.length === 0) {
        throw new ReportError('Query definition must include at least one column');
      }
    }
    
    // Validate columns
    if (!report.columns || report.columns.length === 0) {
      throw new ReportError('Report must include at least one column');
    }
    ...
    if (report.parameters) {
      for (const param of report.parameters) {
        if (!param.name) {
          throw new ReportError('All parameters must have a name');
        }
        
        if (!param.displayName) {
          throw new ReportError('All parameters must have a display name');
        }
        
        if (!param.parameterType) {
          throw new ReportError('All parameters must have a parameter type');
        }
      }
    }
  }
  ...
  private async exportToPdf(
    ...
    throw new ReportError('PDF export not yet implemented');
  }
  ...
  private async exportToExcel(
    ...
    // For now, return a placeholder implementation
    throw new ReportError('Excel export not yet implemented');
  }
  ...
  private async exportToCsv(
    ...
    // For now, return a placeholder implementation
    throw new ReportError('CSV export not yet implemented');
  }
  ...
  private async exportToHtml(
    ...
    // For now, return a placeholder implementation
    throw new ReportError('HTML export not yet implemented');
  }
