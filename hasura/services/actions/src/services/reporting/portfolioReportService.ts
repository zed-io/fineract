...
import {
  ...
  RejectedLoansReport
} from '../../models/reporting/portfolioReports';
 ...
 */
export async function generatePortfolioAtRiskReport(
  asOfDate: string,
  officeId?: string,
  currencyCode?: string,
  includeDetails: boolean = true
): Promise<PortfolioAtRiskReport> {
  logger.info('Generating PAR report', { asOfDate, officeId, currencyCode });

  try {
    // Define PAR brackets
    const parBrackets: ParBracket[] = [
    ...
    const loanOfficerMap: Map<string, ParByLoanOfficer> = new Map();
    const branchMap: Map<string, ParByBranch> = new Map();
    const productMap: Map<string, ParByProduct> = new Map();
    ...
    for (const loan of loans) {
      ...
      const bracket = parBrackets.find(b => 
        overdueDays >= b.daysOverdueFrom && 
      ...
      if (loan.loan_officer_id) {
        ...
        const bracketIndex = parBrackets.findIndex(b => 
          overdueDays >= b.daysOverdueFrom && 
          (b.daysOverdueTo === null || overdueDays <= b.daysOverdueTo)
        );
    ...
    // Build result
    const result: PortfolioAtRiskReport = {
      asOfDate,
      currency,
      totalOutstanding,
      parBrackets,
    ...
    logger.error('Error generating PAR report', { error });
...
export async function generateCollectionReport(
  ...
  logger.info('Generating collection report', { fromDate, toDate, officeId, loanOfficerId });
  ...
  try {
    ...
    const expectedQuery = await db.query(
      `SELECT 
    ...
    const actualQuery = await db.query(
      `SELECT 
    ...
    const dailyCollectionsQuery = await db.query(
    ...
    const productCollectionsQuery = await db.query(
    ...
    const loanOfficerCollectionsQuery = await db.query(
      `WITH officer_expected AS (
        SELECT 
          l.loan_officer_id,
...
export async function generateLoanPortfolioSummary(
  ...
  logger.info('Generating loan portfolio summary', { asOfDate, officeId, currencyCode });
  ...
  try {
    ...
    const disbursementQuery = await db.query(
      `SELECT 
    ...
    const repaymentQuery = await db.query(
    ...
    logger.error('Error generating loan portfolio summary', { error });
 ...
 */
export async function generateExpectedRepaymentsReport(
  fromDate: string,
  toDate: string,
  officeId?: string,
  loanOfficerId?: string,
  currencyCode?: string
): Promise<ExpectedRepaymentsReport> {
  logger.info('Generating expected repayments report', { 
  ...
  try {
    ...
    const dailyScheduleQuery = await db.query(
      `SELECT 
        rs.due_date as date,
