...
export class SavingsCalculationService {
  ...
  async calculateInterest(
    ...
    try {
      ...
      const dailyCalculations: DailyInterestCalculation[] = [];
      ...
      const dailyInterestRate = this.getDailyInterestRate(
      ...
      while (currentDate <= endDate) {
        ...
        const dayTransactions = sortedTransactions.filter(
          tx => tx.transaction_date === dateString && !tx.is_reversed
        );
        ...
        if (currentBalance >= minBalanceForInterest) {
          if (account.interestCalculationType === InterestCalculationType.DAILY_BALANCE) {
            dailyInterest = this.calculateDailyInterest(currentBalance, dailyInterestRate);
          } else if (account.interestCalculationType === InterestCalculationType.AVERAGE_DAILY_BALANCE) {
      ...
      const nextPostingDate = this.calculateNextInterestPostingDate(
        new Date(toDate),
   ...
   */
  calculateNextInterestPostingDate(
    currentDate: Date,
    postingPeriodType: InterestPostingPeriodType
  ): string {
    const result = new Date(currentDate);
    
    switch (postingPeriodType) {
      case InterestPostingPeriodType.MONTHLY:
        // Move to the end of the current month
        result.setMonth(result.getMonth() + 1);
        result.setDate(0); // Last day of the month
        break;
        
      case InterestPostingPeriodType.QUARTERLY:
      ...
      case InterestPostingPeriodType.BIANNUAL:
      ...
      case InterestPostingPeriodType.ANNUAL:
  ...
  isCompoundingDate(
    currentDate: Date,
    compoundingPeriodType: InterestCompoundingPeriodType
  ): boolean {
    const date = currentDate.getDate();
    const month = currentDate.getMonth();
    
    switch (compoundingPeriodType) {
      case InterestCompoundingPeriodType.DAILY:
        // Compound every day
        return true;
        
      case InterestCompoundingPeriodType.MONTHLY:
        // Compound on the last day of the month
        const lastDay = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).getDate();
        return date === lastDay;
        
      case InterestCompoundingPeriodType.QUARTERLY:
      ...
      case InterestCompoundingPeriodType.SEMI_ANNUAL:
      ...
      case InterestCompoundingPeriodType.ANNUAL:
   ...
   */
  private calculateDailyInterest(balance: number, dailyInterestRate: number): number {
   ...
   */
  private getDailyInterestRate(annualInterestRate: number, daysInYearType: number): number {
   ...
   */
  private roundAmount(amount: number, places: number): number {
   ...
   */
  async calculateOverdraftInterest(
    account: SavingsAccount,
    fromDate: string,
    toDate: string,
    transactionData: any[]
    ...
    try {
      ...
      const dailyOverdraftRate = this.getDailyInterestRate(
        overdraftRate,
        account.interestCalculationDaysInYearType
      );
      ...
      logger.error('Error calculating overdraft interest', error);
   ...
   */
  async calculateAccountSummary(account: SavingsAccount, asOfDate: string): Promise<any> {
    try {
      ...
      logger.error('Error calculating account summary', error);
   ...
   */
  private calculateAvailableBalance(account: SavingsAccount): number {
