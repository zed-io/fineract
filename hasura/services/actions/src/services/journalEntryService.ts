 ...
 */
export async function createJournalEntries(input: JournalEntryCreateInput, userId: string) {
  const { 
    officeId, 
    transactionDate, 
    currencyCode, 
  ...
  logger.info('Creating journal entries', { officeId, transactionDate, userId });
  ...
  return db.transaction(async (client) => {
    try {
      ...
      const debitEntryIds = await Promise.all(
        debits.map(debit => 
      ...
      const creditEntryIds = await Promise.all(
        credits.map(credit => 
          createSingleJournalEntry(
            client, 
            credit.glAccountId, 
 ...
 */
export async function reverseJournalEntries(input: JournalEntryReversalInput, userId: string) {
  const { transactionId, comments } = input;
  logger.info('Reversing journal entries', { transactionId, userId });

  return db.transaction(async (client) => {
    try {
      // Find all journal entries with the transaction ID
      const entriesQuery = await client.query(
        `SELECT * FROM fineract_default.gl_journal_entry 
         WHERE transaction_id = $1 AND reversed = false`,
        [transactionId]
      );
      ...
      const reversalIds = await Promise.all(
        entries.map(entry => {
          // Create a reversal entry with opposite type
          const reversalType = entry.type === JournalEntryType.DEBIT ? 
            JournalEntryType.CREDIT : JournalEntryType.DEBIT;
            
          return createSingleJournalEntry(
            client,
            entry.account_id,
            entry.office_id,
            ...
            reversalType,
            entry.amount,
            comments || `Reversal of transaction ${transactionId}`,
            entry.currency_code,
            entry.payment_details_id,
            userId
          );
        })
      );
      ...
      await client.query(
        `UPDATE fineract_default.gl_journal_entry 
         SET reversed = true, 
             reversal_id = $1,
...
export async function getJournalEntries(filters: any = {}) {
  ...
  logger.info('Fetching journal entries', { filters });
  ...
  try {
    ...
    const journalEntries = await db.query(query, queryParams);
 ...
 */
export async function getTrialBalance(criteria: TrialBalanceSearchCriteria): Promise<TrialBalanceResult> {
  const { officeId, runningBalanceOnly, closingBalanceOnly, asOnDate, includeAccumulated } = criteria;
  
  logger.info('Generating trial balance', { criteria });
  ...
  try {
    ...
    const result = await db.query(query, [officeId, endDate]);
    
    // Transform results
    const entries: TrialBalanceEntry[] = result.rows.map(row => ({
 ...
 */
async function createSingleJournalEntry(
  client,
  accountId: string,
  officeId: string,
  ...
  await client.query(
    `INSERT INTO fineract_default.gl_journal_entry(
 ...
 */
async function validateAccounts(client, accountIds: string[]): Promise<void> {
  const uniqueAccountIds = [...new Set(accountIds)];
  
  // Check if all accounts exist and are enabled
  const accountsQuery = await client.query(
    `SELECT id, name, disabled, manual_entries_allowed 
 ...
 */
async function validateOffice(client, officeId: string): Promise<void> {
 ...
 */
async function validateAccountingPeriodOpen(client, officeId: string, transactionDate: string): Promise<void> {
  // Check if there's a GL closure for this office with a date >= the transaction date
  const closureQuery = await client.query(
