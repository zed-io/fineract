...
import { WriteOffDetails, WriteOffStrategy } from '../models/loanAdvanced';
...
export class LoanWriteOffService {
  ...
  async writeLoanOff(
    loanId: string,
    writeOffData: Partial<WriteOffDetails>,
    userId: string
  ): Promise<string> {
    logger.info('Processing loan write-off', { 
      loanId, 
    ...
    return db.transaction(async (client) => {
      try {
        ...
        const loanResult = await client.query(
          `SELECT loan_status, currency_code, principal_outstanding_derived, 
        ...
        const writeOffStrategy = writeOffData.writeOffStrategy || WriteOffStrategy.FULL_OUTSTANDING;
        ...
        switch (writeOffStrategy) {
          case WriteOffStrategy.FULL_OUTSTANDING:
          ...
          case WriteOffStrategy.PRINCIPAL_ONLY:
            // Write off only principal
            principalPortion = loan.principal_outstanding_derived;
            writeOffAmount = principalPortion;
            break;
            
          case WriteOffStrategy.PARTIAL_AMOUNT:
        ...
        logger.error('Error writing off loan', { loanId, error });
   ...
   */
  async undoLoanWriteOff(
    loanId: string,
    transactionId: string,
    userId: string,
    note?: string
  ): Promise<boolean> {
    logger.info('Undoing loan write-off', { 
      loanId, 
      transactionId, 
      userId 
    });
    ...
    return db.transaction(async (client) => {
      try {
        ...
        const txnResult = await client.query(
          `SELECT transaction_date, amount, principal_portion, interest_portion, 
           fee_charges_portion, penalty_charges_portion, is_reversed
           FROM m_loan_transaction 
           WHERE id = $1 AND loan_id = $2 AND transaction_type_enum = 6`,
        ...
        // Get loan details
        const loanResult = await client.query(
          'SELECT loan_status FROM loan WHERE id = $1',
          [loanId]
        );
        ...
        await client.query(
          `UPDATE m_loan_write_off SET
            is_reversed = true,
            reversed_on_date = NOW(),
            reversed_by_user_id = $1,
            reversal_note = $2
           WHERE transaction_id = $3`,
          [userId, note || 'Write-off reversed', transactionId]
        );
        
        return true;
      } catch (error) {
        logger.error('Error undoing loan write-off', { loanId, transactionId, error });
   ...
   */
  async getWriteOffHistory(loanId: string): Promise<Partial<WriteOffDetails>[]> {
    try {
      const result = await db.query(
        `SELECT wo.id, wo.loan_id, wo.write_off_date, wo.write_off_strategy, 
      ...
      logger.error('Error getting write-off history', { loanId, error });
