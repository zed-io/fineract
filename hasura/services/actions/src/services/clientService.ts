...
interface ClientDetailResponse {
  ...
  gender: string | null;
  clientType: string | null;
  clientTypeValue: string | null;
  clientClassification: string | null;
  clientClassificationValue: string | null;
  legalForm: string;
  firstname: string | null;
  middlename: string | null;
  lastname: string | null;
...
interface RejectClientResponse {
  resourceId: string;
  officeId: string;
  clientId: string;
...
interface WithdrawClientResponse {
  resourceId: string;
  officeId: string;
  clientId: string;
...
interface ReactivateClientResponse {
  resourceId: string;
  officeId: string;
  clientId: string;
...
interface ClientAccountsSummaryResponse {
...
interface ClientAddressResponse {
  resourceId: string;
  clientId: string;
  addressType: string;
}
...
export class ClientService {
  ...
  async listClients(params: ClientListParams): Promise<ClientListResponse> {
    ...
    try {
      const queryParams: Array<string | number> = [officeId];
      let query = `
      ...
      logger.error('Error in listClients:', error);
   ...
   */
  async getClientById(clientId: string): Promise<ClientDetailResponse | null> {
    try {
   ...
   */
  async updateClient(clientId: string, data: Partial<ClientData>): Promise<UpdateClientResponse | null> {
    const dbClient = await this.pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      const updateValues: Array<any> = [];
      const updateFields: string[] = [];
      let paramCounter = 1;
      
      // Helper function to add fields to update
      const addUpdateField = (fieldName: string, value: any, dbFieldName?: string) => {
        if (value !== undefined) {
   ...
   * Activate a client
   */
  async activateClient(clientId: string, activationDate: string): Promise<ActivateClientResponse | null> {
    const dbClient = await this.pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      await dbClient.query('ROLLBACK');
      logger.error('Error in activateClient:', error);
   ...
   * Close a client
   */
  async closeClient(clientId: string, closureDate: string, closureReasonId: string): Promise<CloseClientResponse | null> {
    const dbClient = await this.pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      await dbClient.query('ROLLBACK');
      logger.error('Error in closeClient:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   */
  async rejectClient(clientId: string, rejectionDate: string, rejectionReasonId: string): Promise<RejectClientResponse | null> {
    const dbClient = await this.pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      await dbClient.query('ROLLBACK');
      logger.error('Error in rejectClient:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   */
  async withdrawClient(clientId: string, withdrawalDate: string, withdrawalReasonId: string): Promise<WithdrawClientResponse | null> {
    const dbClient = await this.pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      await dbClient.query('ROLLBACK');
      logger.error('Error in withdrawClient:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   */
  async reactivateClient(clientId: string, reactivationDate: string): Promise<ReactivateClientResponse | null> {
    const dbClient = await this.pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      await dbClient.query('ROLLBACK');
      logger.error('Error in reactivateClient:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   */
  async addClientIdentifier(clientId: string, data: IdentifierData): Promise<ClientIdentifierResponse> {
    const dbClient = await this.pool.connect();
    try {
      await dbClient.query('BEGIN');
      ...
      await dbClient.query('ROLLBACK');
      logger.error('Error in addClientIdentifier:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   */
  async getClientAccountsSummary(clientId: string): Promise<ClientAccountsSummaryResponse> {
    try {
      // Check if client exists
      const clientQuery = `SELECT id FROM fineract_default.client WHERE id = $1`;
      const clientResult = await this.pool.query(clientQuery, [clientId]);
      ...
      logger.error('Error in getClientAccountsSummary:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   */
  async addClientAddress(clientId: string, data: AddressData): Promise<ClientAddressResponse> {
    try {
      // Check if client exists
      const clientQuery = `SELECT id FROM fineract_default.client WHERE id = $1`;
      const clientResult = await this.pool.query(clientQuery, [clientId]);
      ...
      logger.error('Error in addClientAddress:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   * Add a family member
   */
  async addFamilyMember(clientId: string, data: FamilyMemberData): Promise<FamilyMemberResponse> {
    try {
      // Check if client exists
      const clientQuery = `SELECT id FROM fineract_default.client WHERE id = $1`;
      const clientResult = await this.pool.query(clientQuery, [clientId]);
      ...
      logger.error('Error in addFamilyMember:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
   ...
   */
  async addClientDocument(clientId: string, data: DocumentData): Promise<DocumentResponse> {
    try {
      // Check if client exists
      const clientQuery = `SELECT id FROM fineract_default.client WHERE id = $1`;
      const clientResult = await this.pool.query(clientQuery, [clientId]);
      ...
      logger.error('Error in addClientDocument:', error);
      if (error instanceof ClientServiceError) {
        throw error;
      }
